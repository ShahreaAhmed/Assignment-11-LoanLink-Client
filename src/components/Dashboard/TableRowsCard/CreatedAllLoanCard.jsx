...
import { BsCheckSquare, BsSquareFill, BsTrash2 } from 'react-icons/bs';
import { FiEdit2 } from 'react-icons/fi';

const CreatedAllLoanCard = ({loan}) => {

    const {image, title, category, interest, maxLoanLimit} = loan
                  ...
                  <tr className="hover:bg-gray-50 dark:hover:bg-slate-800 transition-colors">
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <div className="flex items-center">
                                                <div className="h-12 w-12 flex-shrink-0">
                                                    <img className="h-12 w-12 rounded-lg object-cover" src={image} alt="" />
                                                </div>
                                                <div className="ml-4">
                                                   ...
                                                   <div className="text-[10px] text-gray-400 mt-1">Created By: {loan.createdBy.name}</div>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <span className="text-sm font-semibold text-amber-600 bg-amber-50 dark:bg-slate-800 px-3 py-1 rounded-lg">{interest}%</span>
                                        </td>
                                        ...
                                        {/* {isAdmin && ( */}
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                ...
                                                >
                                                    {loan.showOnHome ? <BsCheckSquare size={14} /> : <BsSquareFill size={14} />}
...
export default CreatedAllLoanCard;